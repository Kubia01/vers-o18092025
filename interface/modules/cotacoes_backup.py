...
class CotacoesModule(BaseModule):
	...
	def create_dados_cotacao_section(self, parent):
		...
		# Tipo de Cotação
		tk.Label(fields_frame, text="Tipo de Cotação *:", 
				 font=('Arial', 10, 'bold'), bg='white').grid(row=row, column=0, sticky="w", pady=5)
		tipo_cot_combo = ttk.Combobox(fields_frame, textvariable=self.tipo_cotacao_var, 
								  values=["Compra", "Locação"], 
								  width=27, state="readonly")
		tipo_cot_combo.grid(row=row, column=1, sticky="ew", padx=(10, 0), pady=5)
		tipo_cot_combo.bind("<<ComboboxSelected>>", self.on_tipo_cotacao_changed)
		row += 1
	...
	def update_produtos_combo(self):
		...
		try:
			modo = self.tipo_cotacao_var.get()
			print(f"DEBUG UPDATE_PRODUTOS: Modo atual: {modo}")
		except Exception as e:
			modo = 'Compra'  # Default se não conseguir obter o modo
			print(f"DEBUG UPDATE_PRODUTOS: Erro ao obter modo, usando default: {e}")
		
		# SEMPRE atualizar o combobox de locação quando esta função é chamada
		print("DEBUG UPDATE_PRODUTOS: Sempre atualizando combobox de locação...")
		try:
			if hasattr(self, 'item_nome_combo_locacao'):
				print("DEBUG UPDATE_PRODUTOS: item_nome_combo_locacao encontrado")
				conn = sqlite3.connect(DB_NAME)
				c = conn.cursor()
				c.execute("SELECT nome FROM produtos WHERE tipo='Produto' AND COALESCE(categoria,'Geral')='Compressores' AND ativo=1 ORDER BY nome")
				compressores = [row[0] for row in c.fetchall()]
				
				print(f"DEBUG UPDATE_PRODUTOS: Encontrados {len(compressores)} compressores no banco")
				
				# Forçar atualização mais agressiva do combobox
				try:
					# Temporariamente mudar para normal para forçar atualização
					self.item_nome_combo_locacao.configure(state='normal')
					
					# Limpar e atualizar valores
					...
					# Forçar redraw do widget
					self.item_nome_combo_locacao.update_idletasks()
					
					print(f"DEBUG UPDATE_PRODUTOS: Atualizados {len(compressores)} compressores para locação")
				except Exception as e:
					print(f"Erro ao atualizar combobox: {e}")
			else:
				print("DEBUG UPDATE_PRODUTOS: item_nome_combo_locacao NÃO encontrado")
		except Exception as e:
			print(f"Erro ao atualizar combo de locação: {e}")
		finally:
			try:
				conn.close()
			except Exception:
		...
		# Se for Locação, sugerir apenas Compressores (lógica original)
		if modo == 'Locação':
			print("DEBUG UPDATE_PRODUTOS: Modo é Locação, lógica original executada")
			return
	...
	def refresh_produtos(self):
		"""Atualizar lista de produtos"""
		print("DEBUG COTAÇÕES: Iniciando refresh_produtos...")
		# Atualizar combo baseado no tipo selecionado
		self.update_produtos_combo()
		
		# SEMPRE forçar atualização do combobox de locação quando um produto é criado
		try:
			modo_atual = self.tipo_cotacao_var.get() if hasattr(self, 'tipo_cotacao_var') else 'N/A'
			print(f"DEBUG COTAÇÕES: Modo atual: {modo_atual}")
			
			# Sempre atualizar o combobox de locação quando um produto é criado
			print("DEBUG COTAÇÕES: Forçando atualização do combobox de locação...")
			if hasattr(self, 'item_nome_combo_locacao'):
				# Forçar refresh do combobox de locação
				self.frame.after_idle(self.force_update_locacao_combo)
				print("DEBUG COTAÇÕES: Atualização agendada para locação")
			else:
				print("DEBUG COTAÇÕES: item_nome_combo_locacao não encontrado")
		except Exception as e:
			print(f"DEBUG COTAÇÕES: Erro ao verificar modo: {e}")
